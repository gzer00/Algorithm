        ...
        for (int j = 0; j < n;j++) {
            if (grid[i][j] != 0 && !visited[i][j]) {    //아직 확인하지 않은 미생물 발견
                ...
                while (!q.empty()) {
                    ...
                    q.pop();
                    for (int k = 0; k < 4; k++) {
                        if (ny + dy[k] >= 0 && ny + dy[k] < n && nx + dx[k] >= 0 && nx + dx[k] < n && grid[ny + dy[k]][nx + dx[k]] == nowM && !visited[ny + dy[k]][nx + dx[k]]) {
                            q.push({ ny + dy[k], nx + dx[k] });
                            visited[ny + dy[k]][nx + dx[k]] = true;
                            mInfo[nowM].push_back({ ny + dy[k] - i, nx + dx[k] - j});
                        }
                    }
                }
            }
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++) {
            ...
            for (auto pos : mInfo[nM]) {
                if (j + pos.second >= n || i + pos.first >= n || nGrid[i + pos.first][j + pos.second] != 0) {
                    wrongPos = true;
                    break;
                }
            }
